...
void MainWindow::showChatMessage(ChatMessage msg){
    ...
    quint32 nowchat = msg.getSender();  // 获取当前消息的发送者ID
    quint32 nowuserinfo = friendInfo->getid();  // 获取当前用户的ID
    ...
    if (nowchat == nowuserinfo) {
        ...
        // 创建新的QListWidgetItem项
        QListWidgetItem *line = new QListWidgetItem;
        line->setSizeHint(QSize(350, height));  // 设置项的尺寸
        ...
        // 获取好友对应的按钮并修改其样式，表示有新消息
        QToolButton* nowbutton = Mapfriend[nowchat];
        nowbutton->setStyleSheet("background-color:rgb(255,0,0); font-size:20px;");
    };
}
